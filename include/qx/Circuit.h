...
public:
    ...
    struct Unitary {
        ...
        // Control bits vector is stored in an optional to avoid allocation in case there are no control bits.
        std::optional<std::vector<core::QubitIndex>> controlBits{};
    };
    
    using Instruction = std::variant<Measure, MeasureAll, PrepZ, Unitary<1>, Unitary<2>, Unitary<3>>;

    // We could in the future add loops and if/else...

    Circuit(std::size_t n, std::string name = "", std::size_t iterations = 1)
...
private:
    std::size_t const numberOfQubits = 0;
