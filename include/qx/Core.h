...
public:
    ...
    explicit constexpr DenseUnitaryMatrix(Matrix const& m) : DenseUnitaryMatrix(m, true) {}

    inline constexpr std::complex<double> at(std::size_t i, std::size_t j) const {
        return matrix[i][j];
    }
...
private:
    constexpr DenseUnitaryMatrix(Matrix const& m, bool checkIsUnitary) : matrix(m) {
        if (checkIsUnitary) {
            checkUnitary();
        }
    }
...
class QuantumState {
public:
    explicit QuantumState(std::size_t n) : numberOfQubits(n), data(1 << numberOfQubits) {
        assert(numberOfQubits > 0 && "QuantumState needs at least one qubit");
        assert(numberOfQubits <= config::MAX_QUBIT_NUMBER && "QuantumState currently cannot support that many qubits with this version of OpenQL");
    ...
    QuantumState& apply(DenseUnitaryMatrix<1 << NumberOfOperands> const& m, std::array<QubitIndex, NumberOfOperands> const& operands);
    ...
    BasisVector getMeasurementRegister() const {
    ...
    void measure(QubitIndex qubitIndex, F&& randomGenerator) {
        ...
        if (rand < probabilityOfMeasuringOne) {
            data.eraseIf([qubitIndex](auto const& kv) { return !kv.first.test(qubitIndex.value); });
    ...
    void prep(QubitIndex qubitIndex);
