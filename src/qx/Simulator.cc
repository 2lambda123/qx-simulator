...
bool Simulator::setString(std::string cqasmData) {
    ...
    try {
        ...
        std::cerr << "Cannot parse and analyze string " << cqasmData << std::endl;
        program.reset();
        return false;
    }
...
std::string Simulator::execute(size_t numberOfRuns) {
    if (program.empty()) {
        std::cerr << "No circuit successfully loaded, call set(...) first" << std::endl;
        return "";
    }
    ...
    for (auto const& subcircuit : subcircuits) {
        perfectCircuits.push_back(loadCqasmCode(qubitCount, *subcircuit));
    }
