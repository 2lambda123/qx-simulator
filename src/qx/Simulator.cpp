...
execute(Program program, Operations const &operations, std::size_t iterations,
    ...
    if (qubitCount <= 64) {
        ...
        throw std::runtime_error(
            "Cannot handle that many qubits in this version of QX-simulator");
    }
...
executeStringImpl(std::string const &s, Operations const &operations,
                  ...
                  std::optional<std::uint_fast64_t> seed) {
    return execute(parseString(s), operations, iterations, seed);
}
...
executeFileImpl(std::string const &filePath, Operations operations,
                std::size_t iterations,
                std::optional<std::uint_fast64_t> seed) {
    return execute(parseFile(filePath), operations, iterations, seed);
}
