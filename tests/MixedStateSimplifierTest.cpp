...
public:
    void checkTriangularization(AbstractMatrix const& input) {
        auto householderAccumulator = EditableSquareMatrix::identity(input.getNumberOfRows());
    ...
    bool areEquals(absl::btree_map<Key<>, std::complex<double>> const& left, absl::btree_map<Key<>, std::complex<double>> const& right) {
...
TEST_CASE_FIXTURE(MixedStateSimplifierTest, "Small Householder matrix") {
    ...
    UnitaryMatrix expectedHouseholder{{-1/std::sqrt(2), -1/std::sqrt(2)}, {-1/std::sqrt(2), 1/std::sqrt(2)}};

    CHECK_EQ(householder, expectedHouseholder);

    m.multiplyLeft(householder);

    Matrix expectedProduct{{-1/std::sqrt(2), -1/std::sqrt(2), 0.}, {0., 0., 0.}};
...
TEST_CASE_FIXTURE(MixedStateSimplifierTest, "Triangularize") {
    ...
    Matrix expectedTriangularized{{-3.1622776601683791, -4.4271887242357311}, {0., 0.63245553203367644}};
...
TEST_CASE_FIXTURE(MixedStateSimplifierTest, "SparseStateAdapter") {
    ...
    SUBCASE("Triangularization does not result in fewer elements") {
        T data {
            {{.ensembleIndex = EnsembleIndex(0), .measurementRegister = {"00"}, .basisVector = {"00"}}, 1.},
            {{.ensembleIndex = EnsembleIndex(0), .measurementRegister = {"00"}, .basisVector = {"10"}}, .5},
            {{.ensembleIndex = EnsembleIndex(1), .measurementRegister = {"00"}, .basisVector = {"10"}}, .5},
            {{.ensembleIndex = EnsembleIndex(1), .measurementRegister = {"10"}, .basisVector = {"00"}}, .2},
            {{.ensembleIndex = EnsembleIndex(1), .measurementRegister = {"10"}, .basisVector = {"01"}}, .1}
        };
    ...
    SUBCASE("Triangularization sparsifies matrix") {
        T data {
            {{.ensembleIndex = EnsembleIndex(0), .measurementRegister = {"10"}, .basisVector = {"00"}}, 1.},
            {{.ensembleIndex = EnsembleIndex(0), .measurementRegister = {"10"}, .basisVector = {"10"}}, .5},
            {{.ensembleIndex = EnsembleIndex(1), .measurementRegister = {"10"}, .basisVector = {"00"}}, .5},
            {{.ensembleIndex = EnsembleIndex(1), .measurementRegister = {"10"}, .basisVector = {"10"}}, .2}
        };

        auto state = SparseStateAdapter<64>::makeIfNeeded(data);
        
        REQUIRE(state.has_value());
        CHECK_EQ(*state, Matrix{
            {1., .5},
            {.5, .2}
        });
        ...
        state->forEach([&](auto i, auto j, auto v) {
            x += v;
        });
...
TEST_CASE_FIXTURE(MixedStateSimplifierTest, "Simplify") {
    using T = absl::btree_map<Key<>, std::complex<double>>;
    T data {
        {{.ensembleIndex = EnsembleIndex(0), .measurementRegister = {"00"}, .basisVector = {"00"}}, .5},
        {{.ensembleIndex = EnsembleIndex(0), .measurementRegister = {"00"}, .basisVector = {"01"}}, .5},
        {{.ensembleIndex = EnsembleIndex(1), .measurementRegister = {"00"}, .basisVector = {"00"}}, .5},
        {{.ensembleIndex = EnsembleIndex(1), .measurementRegister = {"00"}, .basisVector = {"01"}}, .5},
        {{.ensembleIndex = EnsembleIndex(1), .measurementRegister = {"01"}, .basisVector = {"01"}}, .5}
    };
...
TEST_CASE_FIXTURE(MixedStateSimplifierTest, "Sparsify 1" * doctest::skip()) {
    using T = absl::btree_map<Key<>, std::complex<double>>;
    T data {
        {{.ensembleIndex = EnsembleIndex(0), .measurementRegister = {"00"}, .basisVector = {"00"}}, 1 / std::sqrt(2)},
        {{.ensembleIndex = EnsembleIndex(1), .measurementRegister = {"00"}, .basisVector = {"00"}}, 1 / std::sqrt(2)}
    };

    MixedStateSimplifier::sparsify(data);

    CHECK(areEquals(data, T{
        {{.ensembleIndex = EnsembleIndex(0), .measurementRegister = {"00"}, .basisVector = {"00"}}, -1.}
    }));
}
...
TEST_CASE_FIXTURE(MixedStateSimplifierTest, "Sparsify 2" * doctest::skip()) {
    using T = absl::btree_map<Key<>, std::complex<double>>;
    T data {
        {{.ensembleIndex = EnsembleIndex(0), .measurementRegister = {"00"}, .basisVector = {"11"}}, 1 / std::sqrt(3)},
        {{.ensembleIndex = EnsembleIndex(1), .measurementRegister = {"00"}, .basisVector = {"11"}}, 1 / std::sqrt(3)},
        {{.ensembleIndex = EnsembleIndex(3), .measurementRegister = {"00"}, .basisVector = {"11"}}, 1 / std::sqrt(3)}
    };

    MixedStateSimplifier::sparsify(data);

    CHECK(areEquals(data, T{
        {{.ensembleIndex = EnsembleIndex(0), .measurementRegister = {"00"}, .basisVector = {"11"}}, -1.}
    }));
}
...
TEST_CASE_FIXTURE(MixedStateSimplifierTest, "Sparsify 3" * doctest::skip()) {
    ...
    CHECK(areEquals(data, T{
        {{.ensembleIndex = EnsembleIndex(0), .measurementRegister = {"00"}, .basisVector = {"00"}}, -1 / std::sqrt(2)},
        {{.ensembleIndex = EnsembleIndex(0), .measurementRegister = {"00"}, .basisVector = {"01"}}, -1 / std::sqrt(2)}
    }));
}

TEST_CASE_FIXTURE(MixedStateSimplifierTest, "Sparsify doesn't modify" * doctest::skip()) {
    using T = absl::btree_map<Key<>, std::complex<double>>;
    T data {
        {{.ensembleIndex = EnsembleIndex(0), .measurementRegister = {"00"}, .basisVector = {"00"}}, .5},
        {{.ensembleIndex = EnsembleIndex(0), .measurementRegister = {"00"}, .basisVector = {"01"}}, .5},
