...
void SimulationResultAccumulator::dump() {
    ...
    forAllNonZeroStates([](auto const& stateString, auto c) {
        std::cout << stateString << "       " << c << " (" << std::norm(c) << ")" << std::endl;
    });
    ...
    for (const auto &kv: measuredStates) {
        ...
        std::cout << get_state_string(state) << "       " << count << "/" << n_measurements << " (" << static_cast<double>(count) / n_measurements << ")" << std::endl;
    }
}
...
SimulationResult SimulationResultAccumulator::get() {
    ...
    forAllNonZeroStates([&result](auto stateString, auto c) {
        result.state.push_back(std::make_pair(stateString, SimulationResult::Complex{ .real = c.real(), .imag = c.imag() }));
    });
...
void SimulationResultAccumulator::forAllNonZeroStates(F&& f) {
    quantumState.forEach([&f, this](auto const& kv) { f(get_state_string(kv.first), kv.second); });
}
