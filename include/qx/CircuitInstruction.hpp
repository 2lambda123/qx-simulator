...
public:
    ...
    using DynamicOperandsVector =
        absl::InlinedVector<DynamicOperand, config::MAX_INLINED_OPERANDS>;
    using ControlBitsVector =
        absl::InlinedVector<MeasurementRegisterIndex,
                            config::MAX_INLINED_CONTROL_BITS>;
    using KrausOperators = Operations::KrausOperators;

    CircuitInstruction(KrausOperators ks, DynamicOperandsVector o,
                       ControlBitsVector b = {})
        : krausOperators(std::move(ks)), dynamicOperands(std::move(o)),
          controlBits(std::move(b)) {
#ifndef NDEBUG
        Operations::checkValidKrausOperatorSet(
            "unknown", dynamicOperands.size(), krausOperators);
#endif
    }
    ...
    std::complex<double> getKrausValue(KrausOperatorIndex operatorIndex,
