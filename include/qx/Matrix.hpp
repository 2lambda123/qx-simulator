...
public:
    ...
    AbstractMatrix(std::uint64_t n, std::uint64_t m)
    ...
    virtual void forEach(
        std::function<void(std::uint64_t, std::uint64_t, AbstractMatrix::Value)>
            f) const { // FIXME: find a way to avoid std::function
        for (std::uint64_t rowIndex = 0; rowIndex < getNumberOfRows();
             ++rowIndex) {
            for (std::uint64_t colIndex = 0; colIndex < getNumberOfCols();
                 ++colIndex) {
    ...
    virtual bool operator==(AbstractMatrix const &other) const {
        ...
        bool equal = true;
        forEach([&](std::uint64_t rowIndex, std::uint64_t colIndex,
                    AbstractMatrix::Value value) {
            if (utils::isNotNull(other.get(rowIndex, colIndex) - value)) {
                equal = false; // TODO: ability to shortcircuit the loop?
            }
        });

        other.forEach([&](std::uint64_t rowIndex, std::uint64_t colIndex,
                          AbstractMatrix::Value value) {
    ...
    bool isUpperTriangular() {
        bool result = true;
        forEach([&result](std::uint64_t rowIndex, std::uint64_t colIndex,
                          auto value) {
...
public:
    ...
    SubMatrixAdapter(AbstractMatrix &m, Coordinates const &c)
        : AbstractMatrix(c.endRow - c.startRow, c.endCol - c.startCol),
          matrix(m), coordinates(c) {
    ...
    void forEach(
        std::function<void(std::uint64_t, std::uint64_t, AbstractMatrix::Value)>
            f) const override {
        matrix.forEach([&](std::uint64_t rowIndex, std::uint64_t colIndex,
                           AbstractMatrix::Value value) {
            if (rowIndex >= coordinates.startRow &&
                rowIndex <
                    coordinates
                        .endRow // FIXME: not efficient. Maybe forEach should
                                // get begin and end indices optionally?
                                // Iteration should always be ordered.
                && colIndex >= coordinates.startCol &&
                colIndex < coordinates.endCol) {
                f(rowIndex - coordinates.startRow,
                  colIndex - coordinates.startCol, value);
            }
        });
    }
...
inline SubMatrixAdapter extractRow(AbstractMatrix &m, std::uint64_t rowIndex) {
    ...
    SubMatrixAdapter::Coordinates rowCoordinates{.startRow = rowIndex,
                                                 .startCol = 0,
                                                 .endRow = rowIndex + 1,
...
inline SubMatrixAdapter extractCol(AbstractMatrix &m, std::uint64_t colIndex) {
    ...
    SubMatrixAdapter::Coordinates colCoordinates{.startRow = 0,
                                                 .startCol = colIndex,
...
public:
    ...
    virtual void map(std::function<std::optional<AbstractMatrix::Value>(
                         std::uint64_t, std::uint64_t, AbstractMatrix::Value)>
                         f) { // FIXME: find a way to avoid std::function
        forEach([&](std::uint64_t rowIndex, std::uint64_t colIndex,
                    AbstractMatrix::Value v) {
    ...
    virtual void operator*=(AbstractMatrix const &other) {
        ...
        for (std::uint64_t rowIndex = 0; rowIndex < getNumberOfRows();
            ...
            row.forEach([&](std::uint64_t, std::uint64_t colIndex, auto value) {
                for (std::uint64_t otherColIndex = 0;
                     otherColIndex < other.getNumberOfCols(); ++otherColIndex) {
                    newRow[otherColIndex] +=
                        value * other.get(colIndex, otherColIndex);
                }
            });
    ...
    virtual void operator*=(double d) {
        map([d](auto, auto, std::complex<double> v) { return v * d; });
    }
    ...
    void multiplyLeft(AbstractMatrix const &other) {
        ...
        for (std::uint64_t colIndex = 0; colIndex < getNumberOfCols();
            ...
            col.forEach([&](std::uint64_t rowIndex, std::uint64_t, auto value) {
                for (std::uint64_t otherRowIndex = 0;
                     otherRowIndex < other.getNumberOfRows(); ++otherRowIndex) {
                    newCol[otherRowIndex] +=
                        value * other.get(otherRowIndex, rowIndex);
                }
            });
    ...
    virtual void operator+=(AbstractMatrix const &other) {
        ...
        for (std::uint64_t i = 0; i < getNumberOfRows();
             ++i) { // FIXME: forEach?
            for (std::uint64_t j = 0; j < getNumberOfCols(); ++j) {
...
class EditableSubMatrixAdapter : public SubMatrixAdapter,
                                 public EditableAbstractMatrix {
public:
    using SubMatrixAdapter::Coordinates;
    EditableSubMatrixAdapter(EditableAbstractMatrix &m, Coordinates const &c)
        : AbstractMatrix(c.endRow - c.startRow, c.endCol - c.startCol),
          SubMatrixAdapter(m, c) {}

    void set(std::uint64_t i, std::uint64_t j,
             AbstractMatrix::Value v) override {
        assert(i < getNumberOfRows());
        assert(j < getNumberOfCols());
        return dynamic_cast<EditableAbstractMatrix &>(matrix).set(
            i + coordinates.startRow, j + coordinates.startCol, v);
    }

    void
    map(std::function<std::optional<Value>(std::uint64_t, std::uint64_t, Value)>
            f) override {
        dynamic_cast<EditableAbstractMatrix &>(matrix).map(
            [&](std::uint64_t rowIndex, std::uint64_t colIndex, Value value) {
                if (rowIndex >= coordinates.startRow &&
                    rowIndex < coordinates.endRow &&
                    colIndex >= coordinates.startCol &&
                    colIndex < coordinates.endCol) {
                    return f(rowIndex - coordinates.startRow,
                             colIndex - coordinates.startCol, value);
                }
...
public:
    Matrix(std::uint64_t n, std::uint64_t m)
        : AbstractMatrix(n, m),
          matrix(getNumberOfRows() * getNumberOfCols(), 0.) {}
    ...
    explicit Matrix(AbstractMatrix const &m)
        : Matrix(m.getNumberOfRows(), m.getNumberOfCols()) {
        m.forEach(
            [&](std::uint64_t rowIndex, std::uint64_t colIndex, auto value) {
    ...
    inline Value get(std::uint64_t i, std::uint64_t j) const override {
        assert(i < getNumberOfRows());
        assert(j < getNumberOfCols());
        assert(i * getNumberOfCols() + j <
               getNumberOfRows() * getNumberOfCols());
        return matrix[i * getNumberOfCols() + j];
    }

    void forEach(std::function<void(std::uint64_t, std::uint64_t, Value)> f)
        const override {
        std::uint64_t i = 0;
        ...
        for (std::uint64_t i = 0; i < getNumberOfRows(); ++i) {
            for (std::uint64_t j = 0; j < other.getNumberOfCols(); ++j) {
                for (std::uint64_t k = 0; k < getNumberOfCols(); ++k) {
                    m.matrix[i * m.getNumberOfCols() + j] +=
...
protected:
    absl::InlinedVector<std::complex<double>,
                        config::MAX_INLINED_MATRIX_ELEMENTS>
        matrix;
};
...
public:
    EditableMatrix(std::uint64_t n, std::uint64_t m)
        : AbstractMatrix(n, m), Matrix(n, m) {}

    EditableMatrix(std::uint64_t n) : AbstractMatrix(n), Matrix(n) {}

    EditableMatrix(AbstractMatrix const &m)
        : AbstractMatrix(m.getNumberOfRows(), m.getNumberOfCols()), Matrix(m) {}

    explicit EditableMatrix(
        std::initializer_list<std::initializer_list<std::complex<double>>> init)
        : AbstractMatrix(init.size(),
                         init.begin() == init.end() ? 0 : init.begin()->size()),
          Matrix(init){};
...
public:
    ...
    SquareMatrix(Matrix const &m)
        : AbstractMatrix(m.getNumberOfRows()), Matrix(m) {
...
public:
    ...
    explicit EditableSquareMatrix(
        std::initializer_list<std::initializer_list<std::complex<double>>> init)
        : AbstractMatrix(init.size()), Matrix(init), SquareMatrix(init),
          EditableMatrix(init) {}
};

class UnitaryMatrix : public SquareMatrix {
public:
    explicit UnitaryMatrix(
        std::initializer_list<std::initializer_list<std::complex<double>>> init)
        : AbstractMatrix(init.size(),
                         init.size() == 0 ? 0 : init.begin()->size()),
          Matrix(init), SquareMatrix(init) {
        if (!(*this * SquareMatrix::dagger() ==
              SquareMatrix::identity(getSize()))) {
...
public:
    HouseholderMatrix(AbstractMatrix const &vector)
        ...
        vector.forEach([&vectorSquaredNormWithoutFirstElement](
                           std::uint64_t rowIndex, std::uint64_t colIndex,
                           AbstractMatrix::Value value) {
            assert(colIndex == 0);
            if (rowIndex == 0) {
                return;
        ...
        double vectorNorm = std::sqrt(vectorSquaredNormWithoutFirstElement +
                                      std::norm(vector.get(0, 0)));

        u.set(0, 0,
              vector.get(0, 0) +
                  vector.get(0, 0) / std::abs(vector.get(0, 0)) * vectorNorm);
        for (std::uint64_t rowIndex = 1; rowIndex < u.getNumberOfRows();
             ++rowIndex) {
            u.set(rowIndex, 0, vector.get(rowIndex, 0));
        }

        double uSquaredNorm =
            vectorSquaredNormWithoutFirstElement + std::norm(u.get(0, 0));
    ...
    AbstractMatrix::Value get(std::uint64_t i, std::uint64_t j) const override {
        ...
        if (uInvSquaredNorm) {
            value -= 2 * uInvSquaredNorm.value() * u.get(i, 0) *
                     std::conj(u.get(j, 0));
        }
...
public:
    MatrixExtender(AbstractMatrix const &m, std::uint64_t size)
        : AbstractMatrix(size, size), matrix(m),
...
inline void qrTriangularize(EditableAbstractMatrix &matrix,
                            EditableAbstractMatrix *auxiliary = nullptr) {
    SubMatrixAdapter::Coordinates subMatrixCoordinates{
        .startRow = 0,
        .startCol = 0,
        .endRow = matrix.getNumberOfRows(),
        .endCol = matrix.getNumberOfCols()};
    while (subMatrixCoordinates.startRow <
           std::min(matrix.getNumberOfRows(), matrix.getNumberOfCols())) {
        ...
        if (auxiliary) {
            auxiliary->multiplyLeft(MatrixExtender(
                householderMatrix, auxiliary->getNumberOfRows()));
        }
...
inline double computeSpectralRadius(AbstractMatrix const &matrix) {
    ...
    for (std::uint64_t i = 0; i < v.getNumberOfRows(); ++i) {
        v.set(i, 0,
              std::complex<double>{random::randomZeroOneDouble(),
