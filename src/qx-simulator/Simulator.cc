...
int main(int argc, char **argv) {
    ...
    if (filePath == "" || argParsingFailed) {
        std::cerr << "Usage: \n   " << argv[0] << " [-j filename.json] [-c iterations] file.qc" << std::endl;
        return -1;
    }

    if (jsonFilename != "") {
        std::cout << "Will output JSON simulation result to file " << jsonFilename << std::endl;
    }
    
    std::cout << "Will execute circuit " << numberOfRuns << " time" << (numberOfRuns > 1 ? "s" : "")  << std::endl;
